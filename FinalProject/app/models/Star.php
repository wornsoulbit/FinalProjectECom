...
class Star extends \App\core\Model {
    ...
    public function find($profile_id, $page_id) {
        $stmt = self::$connection->prepare("SELECT * FROM star_page WHERE profile_id = :profile_id AND page_id=:page_id");
    ...
    public function findPage($page_id) {
        $stmt = self::$connection->prepare("SELECT * FROM star_page WHERE page_id=:page_id");
    ...
    public function delete() {
        $stmt = self::$connection->prepare("DELETE FROM star_page WHERE page_id=:page_id AND profile_id=:profile_id");
        $stmt->execute(['page_id' => $this->page_id, 'profile_id' => $this->profile_id]);
    }

    public function deleteAll() {
        $stmt = self::$connection->prepare("DELETE FROM star_page WHERE page_id=:page_id");
